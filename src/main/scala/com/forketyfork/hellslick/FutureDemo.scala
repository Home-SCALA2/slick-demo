...
object FutureDemo extends App {
  ...
  def getFuture1: Future[List[String]] = Future {
    List("1337", "1111", "2222", "3333")
  }

  // вторая футура из строки делает число
  def getFuture2(nums: List[String]): Future[List[Int]] = Future { //todo  это `Future` - класс который позволяет получать результаты (не сразу, а) дожидаясь окончания выполнения...
//    num.toInt
